...
impl ImplementationLevelBuilder {
    ...
    async fn design_skeleton(&self, plan: &GenerationPlan) -> Result<RepoSkeleton> {
        let components: String = plan.component_plan.components
            .iter()
        ...
        let response: SkeletonResponse = self.client
    ...
    fn response_to_skeleton(&self, response: SkeletonResponse, language: &crate::types::TargetLanguage) -> RepoSkeleton {
        let mut skeleton = RepoSkeleton::new(PathBuf::from("."), language.as_str());
        
        for file in response.files {
            let skeleton_file = SkeletonFile::new(
    ...
    async fn design_interfaces(
        ...
        for file in &skeleton.files {
            ...
            let interface = self.design_file_interface(
                &file.path,
                &component_name,
                &features,
    ...
    async fn design_file_interface(
        ...
        let response: InterfaceResponse = self.client
            .complete_json_raw("", &prompt)
            .await
            .and_then(|v| self.client.deserialize(v))?;
    ...
    fn response_to_interface(&self, path: PathBuf, response: InterfaceResponse) -> FileInterface {
        let units: Vec<UnitInterface> = response.units
            .into_iter()
            .map(|u| UnitInterface {
                name: u.name,
    ...
    fn find_features_for_component(&self, component_name: &str, plan: &GenerationPlan) -> Vec<String> {
        plan.component_plan.components
            .iter()
    ...
    fn plan_tasks(
        ...
        for component in &plan.component_plan.components {
            let files: Vec<_> = skeleton.files
                .iter()
                .filter(|f| {
                    let component_dir = component.name.replace('.', "/");
    ...
    fn create_task_for_file(
        ...
        let mut task = ImplementationTask::new(
            &format!("task_{}", path.display().to_string().replace(['/', '.'], "_")),
            path.to_path_buf(),
            component,
        );
