...
impl OpenAIClient {
    ...
    pub fn deserialize<T: serde::de::DeserializeOwned>(&self, value: serde_json::Value) -> Result<T> {
...
impl LlmClient for OpenAIClient {
    ...
    async fn complete(&self, system: &str, user: &str) -> Result<String> {
        let _permit = self.semaphore.acquire().await
            .map_err(|e| GeneratorError::Infrastructure(std::io::Error::other(
                e.to_string()
            )))?;
...
mod tests {
    ...
    fn test_extract_json() {
        ...
        let json: serde_json::Value = client.extract_json(
            r#"Some text before {"key": "value"} and after"#
        ).unwrap();
        assert_eq!(json["key"], "value");
    }
}
